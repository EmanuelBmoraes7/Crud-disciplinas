"""
Step definitions para todos os cenários BDD de disciplinas.
"""
...
# ── Carrega todos os cenários dos arquivos .feature ──────────────────────────
scenarios(
...
@pytest.fixture
def contexto():
    """Dicionário compartilhado para guardar estado entre steps."""
    return {}
...
@given(parsers.parse("que as seguintes disciplinas estão cadastradas:\n{tabela}"))
def disciplinas_pre_cadastradas(gerenciador, contexto, tabela):
    contexto["gerenciador"] = gerenciador
    linhas = [l.strip() for l in tabela.strip().splitlines() if l.strip()]
    # pula cabeçalho
    for linha in linhas[1:]:
        partes = [p.strip() for p in linha.split("|") if p.strip()]
        titulo, inicio_str, termino_str, vagas_str, verao_str = partes
        gerenciador.criar(
            titulo=titulo,
            data_inicio=date.fromisoformat(inicio_str),
            data_termino=date.fromisoformat(termino_str),
            num_vagas=int(vagas_str),
            e_verao=(verao_str.lower() == "verdadeiro"),
        )
...
@when(
    parsers.parse(
        'eu criar a disciplina com título "{titulo}", início "{inicio}", '
        'término "{termino}", vagas {vagas:d} e verão {verao}'
    )
)
...
@when(
    parsers.parse(
        'eu tentar criar uma disciplina com início "{inicio}" e término "{termino}"'
    )
)
...
@when(
    parsers.parse(
        'eu atualizar a disciplina de ID {id:d} com início "{inicio}" e término "{termino}"'
    )
)
...
@when(
    parsers.parse(
        'eu tentar atualizar a disciplina de ID {id:d} com início "{inicio}" e término "{termino}"'
    )
)
...
@when("eu listar todas quando listar todas")
def listar_todas_alias(gerenciador, contexto):
    contexto["lista"] = gerenciador.listar_todas()
...
def ids_diferentes(contexto):
    disciplinas = contexto["gerenciador"].listar_todas()
...
def disciplina_titulo_atualizado(contexto, id, titulo):
    d = contexto["gerenciador"].buscar_por_id(id)
    assert d.titulo == titulo


@then(parsers.parse("o número de vagas da disciplina de ID {id:d} deve ser {vagas:d}"))
def disciplina_vagas_atualizadas(contexto, id, vagas):
    d = contexto["gerenciador"].buscar_por_id(id)
    assert d.num_vagas == vagas


@then(parsers.parse("a disciplina de ID {id:d} deve ser marcada como de verão"))
def disciplina_verao_atualizado(contexto, id):
    d = contexto["gerenciador"].buscar_por_id(id)
    assert d.e_verao is True


@then(parsers.parse('a data de início da disciplina de ID {id:d} deve ser "{inicio}"'))
def disciplina_inicio_atualizado(contexto, id, inicio):
    d = contexto["gerenciador"].buscar_por_id(id)
    assert str(d.data_inicio) == inicio


@then(parsers.parse('a data de término da disciplina de ID {id:d} deve ser "{termino}"'))
def disciplina_termino_atualizado(contexto, id, termino):
    d = contexto["gerenciador"].buscar_por_id(id)
    assert str(d.data_termino) == termino
...
def disciplina_nao_existe(contexto, id):
    with pytest.raises(DisciplinaError):
        contexto["gerenciador"].buscar_por_id(id)


@then(parsers.parse("a disciplina de ID {id:d} ainda deve existir"))
def disciplina_ainda_existe(contexto, id):
    d = contexto["gerenciador"].buscar_por_id(id)
    assert d is not None


@then("devo receber uma lista vazia quando listar todas")
def lista_vazia_apos_delecao(contexto):
    assert contexto["gerenciador"].listar_todas() == []
